...
export const validateFile = (
    ...
    if (sizeBetween && (file.size < sizeBetween[0] || file.size > sizeBetween[1])) {
    ...
    if (types) {
        const requiredMimeTypes = imageMimeTypes.filter(extenstion => types.includes(extenstion))

        const isValidType = requiredMimeTypes.some(mimeType => file.type === mimeType)

        if (!isValidType) {
            errors.types = new ValidationError(
                errorMessages?.types?.(types) ?? `The file should be of type ${types.join(', ')}`,
            )
        }
    }

    if (extensions) {
        const requiredExtensions = extensions.filter(extenstion => extensions.includes(extenstion))

        const isValidExtension = requiredExtensions.some(extension => file.name.endsWith(extension))

        if (!isValidExtension) {
            errors.extensions = new ValidationError(
                errorMessages?.extensions?.(extensions) ?? `The file should have extension ${extensions.join(', ')}`,
            )
        }
    }
